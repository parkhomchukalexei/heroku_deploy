...
class AnastasiaDatingWorkpage(LoginRequiredMixin,View):
    ...
    permission_required = 'anastasiadating.view_anastasiadatingtable'
    template = 'anastasiadating_template/workpage.html'

    def __init__(self):
        self.data = ViewData(table_object=AnastasiaDatingTable, table_serializer=TableSerializer, table_data_object=TableData,
    ...
    def create_pagination_object(self, user):
        ...
        for i in range(1, 13):
            if i < 10:
                ...
                month = str(i)
            if '[AD] Operator' in str(user.groups.all()):
                data.append(
    ...
    def get(self, request):
        if '[AD] Operator' in str(request.user.groups.all()):
...
class CreateNewTable(LoginRequiredMixin, View):
    permission_required = 'anastasiadating.add_anastasiadatingtable'
...
class TableDataSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return HttpResponseRedirect(redirect_to=f'http://127.0.0.1:8000/anastasiadating/?page={url}')
        else:
